...
#define PushB1  10 // Pin for push button 1  
...
int day_limit=800, night_limit=775; // analog reading levels corresponding to switching from day to night - difference used to avoid toggling between two levels when light level is borderline
...
#define ACCELEROMETER_ORIENTATION 1     // 0, 1, 2, 3 or 4 to set the orientation of the accerometer module
...
void CalibrateAccel(){
  ...
  for (int i=0; i<100;i++){
    ...
    a_forward = (ACCELEROMETER_ORIENTATION == 0?-ax:(ACCELEROMETER_ORIENTATION == 1?-ax:(ACCELEROMETER_ORIENTATION == 2?-ax:(ACCELEROMETER_ORIENTATION == 3?-ay:ay))))/164;
    a_sideway = (ACCELEROMETER_ORIENTATION == 0?ay:(ACCELEROMETER_ORIENTATION == 1?az:(ACCELEROMETER_ORIENTATION == 2?-az:(ACCELEROMETER_ORIENTATION == 3?az:-az))))/164;
    a_vertical = (ACCELEROMETER_ORIENTATION == 0?az:(ACCELEROMETER_ORIENTATION == 1?-ay:(ACCELEROMETER_ORIENTATION == 2?ay:(ACCELEROMETER_ORIENTATION == 3?ax:ax))))/164;
